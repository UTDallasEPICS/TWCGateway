...
function ArchivedUsers({ userRole }) {
    ...
    const handleRestoreClick = async (id) => {
        try {
            const response = await axios.put(`http://localhost:5010/user/archived/${id}`, {
                archived: false,
            })
            fetchUsers()
        } catch (error) {
    ...
    const handleRestoreAllClick = async () => {
        try {
            const response = await axios.put(`http://localhost:5010/users/archived`, {
                archived: false,
            })
            fetchUsers()
        } catch (error) {
                    ...
                    >
                        <AlertDialogOverlay>
                            <AlertDialogContent>
                                ...
                                <AlertDialogBody>
                                    This will delete all users permanently. This action cannot be
                                    undone.
                                </AlertDialogBody>
                                ...
                                <AlertDialogFooter>
                                    ...
                                    <Button
                                        colorScheme="red"
                                        onClick={handleDeleteAllConfirmClick}
                                        ml={3}
                                    >
                                        Delete
                                    </Button>
                                ...
                                .filter(
                                    (user) =>
                                        ...
                                        user.roleName
                                            .toLowerCase()
                                            .includes(searchTerm.toLowerCase()) ||
                                        user.departmentName.some((departmentName) =>
                                            departmentName
                                                .toLowerCase()
                                                .includes(searchTerm.toLowerCase())
                                        )
                                ...
                                .map((user, index) => (
                                    <>
                                        ...
                                        >
                                            ...
                                            <Td>
                                                {user.departmentName &&
                                                user.departmentName.length > 0
                                                    ? user.departmentName.join(', ')
                                                    : 'NA'}
                                            </Td>
