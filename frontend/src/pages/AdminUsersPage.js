...
const AdminUsersPage = () => {
  ...
  const COLUMNS = [
    ...
    {
      label: 'Department',
      renderCell: item => item.departmentName.map((dept, index) => <span key={index}>{dept}<br /></span>)
    },
    // { label: 'Edit', renderCell: item => item.edit },
  ];
  //useEffect
  React.useEffect(() => {
    axios
  ...
  return (
    <div className="flex">
      ...
      <div className="flex flex-col flex-grow">
        ...
        <Section title="Onboarding Employees">
          <CompactTable columns={COLUMNS} data={{ nodes: employees }} theme={theme} />
        </Section>
      </div>
    </div>
  );
