...
module.exports = {   
    getAllDepartments: async (req, res) => {
        try {
            ...
            const departmentsWithUsers = await Promise.all(departments.map(async (department) => {
                const userMappings = await prisma.departmentUserMapping.findMany({
                    where: {
                        departmentId: department.id
                    }
                });
                const users = await Promise.all(userMappings.map(async (mapping) => {
            ...
            const departmentsWithTasks = await Promise.all(departmentsWithUsers.map(async (department) => {
                const taskMappings = await prisma.departmentTaskMapping.findMany({
                    where: {
                        departmentId: department.id
                    }
                });
                const tasks = await Promise.all(taskMappings.map(async (mapping) => {
