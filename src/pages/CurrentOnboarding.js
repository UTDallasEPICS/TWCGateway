...
const CurrentOnboarding = () => {
  ...
  const fetchDB = async () => {
    const response = await fetch(
      "http://localhost:5001/displayEmployeeTaskGroup/" + accountID
    );

    const response2 = await fetch(
      "http://localhost:5001/displayDepartmentTaskGroups/" + depName
    );
  ...
  if (typeof dataBase.rowCount !== "undefined") {
    ...
    var taskList = taskFillerForMultipleEmployees(
      dataBase,
      dataBase.rowCount / numTasks
    );
    console.log("PAssing ok name?" + user.userName);
    var elements = displayFillerMultipleEmployees(taskList, user.userName);
  }
  /*
...
async function getTaskOwnerName(emp_id) {
  try {
    ...
    const results = await fetch(
      "http://localhost:5001/getEmployeeName/" + emp_id
    );
...
function displayFillerSingleEmployee(taskList, userNameSeessionVariable) {
  try {
    //const { user } = useAuth0();
    console.log("SESSION VERIABLE: " + userNameSeessionVariable);
        ...
        <tr>
          ...
          <td>
            <button
              ...
              onClick={() =>
                confirm(
                  userNameSeessionVariable,
                  taskList[i].assigned_employee_id,
                  taskList[i].task_num
                )
              }
            >
...
function displayFillerMultipleEmployees(employeeTasks, userNameLocal) {
  try {
          ...
          <Card>
            ...
            <CardBody className="">
              <Table bordered striped>
                ...
                <tbody>
                  {displayFillerSingleEmployee(employeeTasks[i], userNameLocal)}
                </tbody>
              </Table>
            </CardBody>
          </Card>
